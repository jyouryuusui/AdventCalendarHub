<template>
  ...
  <v-data-table
    ...
    <template v-slot:item.articleId="{ item }">
      <div>
        <span class="creationDate">{{ item.raw.creationDate }}</span>
        <span class="authorname" style="margin:0 12px;" @click="() => handleAuthorClick(item.raw.author.name)">{{ item.raw.author.name }}</span>
        ...
        <div class="titlediv">
          <v-avatar  style="cursor: pointer;margin-right: 4px;margin-top:-4px;" size="20">
            <v-img v-if="locale=='en'" :src="'icon/'+item.raw.author.icon"></v-img>
